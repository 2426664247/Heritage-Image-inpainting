...
网页 UI 提供完整的单图交互式修复流程：上传原图、绘制多边形病害区域或直接导入黑白掩码、设置推理参数、选择是否使用 LoRA，并在运行结束后查看拟修复区域与结果对比图。
...
2. 上传一张待修复原图。
3. 选择掩码输入方式：可以在图像上依次点击病害区域边界（至少 3 个点后点击“完成区域”），也可以点击“导入掩码”直接选择已有黑白掩码。导入掩码中白色表示修复区、黑色表示保留区。
4. 如果同时导入掩码并绘制了多边形，两部分会合并为最终修复区域；尺寸不一致的导入掩码会使用最近邻方式自动对齐原图。
5. 按需填写提示词，并设置采样步数、模型输入尺寸、提示词引导强度和随机种子。
6. 按需选择是否使用部分微调 UNet，以及是否使用当前数据集快速训练本次实验专属 LoRA。两个开关可以独立组合。
7. 点击“开始修复”，右侧会持续显示当前阶段、进度和简化日志。
8. 完成后可在页面中查看结果对比图，并分别打开拟修复区域示意图和结果对比图。

如果要复现实验，可直接点击“导入历史实验”并选择 `outputs/ui/jobs/<实验名称>/` 文件夹。页面会一次恢复该目录中的 `input.png`、`mask.png` 与 `request.json`：原图、最终掩码、实验名称、提示词、采样步数、尺寸、引导强度、随机种子以及两个模型开关都会自动填回。再次点击“开始修复”会按同名覆盖规则更新该实验目录。
...
outputs/ui/jobs/<实验名称>/
├── input.png             # 本次上传的原图
├── mask.png              # 绘制区域与导入掩码合并后的最终二值掩码；白色为修复区
├── repair_area.png       # 拟修复区域示意图；修复区以纯白色覆盖在原图上
├── result.png            # 模型生成的单独修复结果
├── collage.png           # 原图、拟修复区域、修复结果三栏对比图
├── request.json          # 实验名称、推理参数、掩码来源和多边形坐标
└── training/             # 仅快速训练模式生成
    └── lora_unet.safetensors
```

`repair_area.png` 只用于确认修复范围，不会作为模型输入；模型实际读取的是 `mask.png`。页面绘制或导入的白色区域为修复区、黑色区域为保留区。导入掩码会在页面上以半透明白色预览，便于与原图核对。
...
- v0.2.0
  ...
  - 网页支持直接导入黑白掩码，并可与手绘多边形区域合并，便于开展统一参数下的对比实验。
  - 新增“导入历史实验”，选择已有实验文件夹即可一键恢复原图、最终掩码和全部运行参数。
